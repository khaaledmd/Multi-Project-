...
def _fig2_ycbcr(fig, spec, frame_bgr):
    ...
    for i, (title, data, cmap) in enumerate(panels):
        ax = fig.add_subplot(inner[i])
        ax.imshow(data, cmap=cmap, vmin=0 if cmap else None, vmax=255 if cmap else None)
        ax.set_title(title, fontsize=9, color="white", pad=4)
        ax.axis("off")


# ── Sous-figure 3 : DCT pipeline ─────────────────────────────
...
def _fig3_dct(fig, spec, orig_frames, encoded_frames):
    ...
    # Extraire un bloc 8×8 au centre de la première frame
...
# ── Sous-figure 4 : vecteurs de mouvement ────────────────────
...
# ── Sous-figure 5 : résidus + graphe PSNR ────────────────────
...
def _fig5_psnr(fig, spec, orig_frames, recon_frames,
    ...
    ax_p.set_xlabel(f"Ratio: {metrics['ratio']:.2f}×  |  "
                    f"{metrics['i_frames']} I + {metrics['p_frames']} P frames",
...
def experiment_quality(frames_dir, max_frames=20, gop=5, search=8, output_dir="output"):
    """
    Relance l'encodage avec plusieurs Quality factors.
    Produit : output/graph_quality.png
    Quality values testées : 10, 20, 30, 50, 70, 90
    → Pour changer : modifie la liste QUALITY_VALUES ci-dessous
    """
    from inter_coding import encode_gop, decode_gop

    QUALITY_VALUES = [10, 20, 30, 50]   # ← change ici si besoin
    ...
    print("\n[Expérience 1] Compression Ratio vs Quality Factor")
    print("-" * 45)
    for q in QUALITY_VALUES:
        ...
        ratio    = orig_sz / comp_sz
        ratios.append(ratio)
        print(f"  Quality={q:3.0f} → ratio={ratio:.2f}×")
    ...
    ax.plot(QUALITY_VALUES, ratios, "o-", color="#58a6ff", lw=2.5, ms=8)
    ...
    ax.set_xlabel("Quality Factor", color="white", fontsize=12)
    ...
    plt.close()
    print(f"[OK] Graphe sauvegardé : {save}")


def experiment_gop(frames_dir, max_frames=20, quality=50, search=8, output_dir="output"):
    """
    Relance l'encodage avec plusieurs GOP sizes.
    Produit : output/graph_gop.png
    GOP values testées : 1, 2, 4, 5, 10, 20
    → Pour changer : modifie la liste GOP_VALUES ci-dessous
    """
    from inter_coding import encode_gop

    GOP_VALUES = [1, 2, 4, 5, 10]  # ← change ici si besoin
    ...
    print("\n[Expérience 2] Compression Ratio vs GOP Size")
    print("-" * 45)
    for gop in GOP_VALUES:
        ...
        ratio   = orig_sz / comp_sz
        ratios.append(ratio)
        print(f"  GOP={gop:3d} → ratio={ratio:.2f}×")
    ...
    ax.plot(GOP_VALUES, ratios, "D-", color="#3fb950", lw=2.5, ms=8)
    ...
    ax.set_ylabel("Compression Ratio (×)", color="white", fontsize=12)
    ax.set_title("Impact du GOP Size sur la Compression",
                 color="white", fontsize=14, fontweight="bold")
    ax.tick_params(colors="white")
    for sp in ax.spines.values(): sp.set_color("#30363d")
...
if __name__ == "__main__":
    ...
    print("[1/3] Chargement des frames...")
